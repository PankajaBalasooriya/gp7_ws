...
from gp7_control.kinematics import inverse_kinematics_pos
...
class PickAndPlaceNode(Node):
    ...
    def __init__(self):
        ...
        self.box_B_xy = np.array([0.6, 0.4])
        self.box_A_xy = np.array([0.4, 0.3])
    ...
    def run_sequence_once(self):
        ...
        for i, p in enumerate(waypoints_cart):
            ...
            q_sol, ok = inverse_kinematics_pos(p, q_init=q_current)
            if not ok:
                self.get_logger().error(f"IK (position) failed at waypoint {i}")
                return
